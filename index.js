...
const findConnectedNodes = (node, vertices, edges) => {
    let connectedVertices = []
    let connectedNames = edges.filter( edge => edge.includes(node.name) )
    let connections = connectedNames.map( cn =>
        cn.filter( n => n !== node.name)[0])
        connections.forEach( conn => vertices.forEach( vert => {
            if (vert.name === conn && vert.discovered === null) connectedVertices.push(vert)
        }))
